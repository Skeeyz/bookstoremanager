...
//	}
	 ...
	 * Create the frame.
	 */
	public DLG_ChonKhuyenMai(QL_BanHang parent) {
		...
		tbl_dsKM.setModel(new DefaultTableModel(
			new Object[][]{},
			new String[] {
				"Mã KM", "Chương Trình", "Ngày BĐ", " Ngày KT", "Phần Trăm Giảm", "Điều kiện", "Tình trạng"
			}
		));
	...
	private void clickTableKhuyenMai() {
		...
		if (row > -1) {
			...
			String end = tbl_dsKM.getValueAt(row, 3) + "";
                        String pt = tbl_dsKM.getValueAt(row, 4) + "";
			String dk = tbl_dsKM.getValueAt(row, 5) + "";
			String trangthai = tbl_dsKM.getValueAt(row, 6) + "";
			...
			KMTimThay.setNgayKT(ngayKT);
                        KMTimThay.setPhanTramGiam(Integer.parseInt(pt.trim()));
                        KMTimThay.setDieuKien(Integer.parseInt(dk.trim()));
		}
	}
	private void chonKhuyenMai() {
		...
		parent.setKhuyenMai(String.valueOf(KMTimThay.getMaKM()),KMTimThay.getTenKM());
	}
