...
public class NhanVienBUS {
    ...
    public boolean themNhanVien(String ho, String ten, String gioiTinh, String chucVu) {
        ...
        if(ten.length()>50 || ho.length()>50){
            new MyDialog("Họ hoặc tên không được nhập quá 50 kí tự!", MyDialog.ERROR_DIALOG);
            return false;
        }
        ho = ho.trim();
        ten = ten.trim();
        chucVu = chucVu.trim();
        MyDialog dlg = new MyDialog("Bạn chắc chắn muốn thêm nhân viên với thông tin này?", MyDialog.WARNING_DIALOG);
    ...
    public boolean updateNhanVien(String ma, String ho, String ten, String gioiTinh, String chucVu) {
        ...
        ten = ten.trim();
        chucVu = chucVu.trim();
        MyDialog dlg = new MyDialog("Bạn chắc chắn muốn sửa thông tin của khách hàng này?", MyDialog.WARNING_DIALOG);
        if(dlg.getAction() == MyDialog.CANCEL_OPTION)
            return false;
        NhanVien nv = new NhanVien();
        nv.setMaNV(maNV);
